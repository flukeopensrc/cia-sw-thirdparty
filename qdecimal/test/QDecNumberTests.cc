...
int QDecNumberTests::opTest(const QStringList& tokens)
{
  ...
  if(ret) {
    ...
    qDebug() << "n1=" << n1.toString().data()
             << "n2=" << n2.toString().data()
             << "r="
             << (is_rs_used ? rs.toLatin1().data() : r.toString().data())
             << "e=" << e.toString().data()
             << "prc=" << oc.digits()
...
bool QDecNumberTests::token2QDecNumber(const QString& token, const QDecContext& ctx, QDecNumber& num)
{
  ...
  if(token.contains('#')) {
    ...
    if(expl.exactMatch(token)) {
      ...
      switch(hexval.size()) {
        case 8: {
          QDecSingle ds;
          ds.fromHexString(hexval.toLatin1().data());
          num = ds.toQDecNumber();
          return true;
          }      
        case 16: {
          QDecDouble dd;
          dd.fromHexString(hexval.toLatin1().data());
          num = dd.toQDecNumber();
          return true;
        }
        case 32: {
          QDecQuad dq;
          dq.fromHexString(hexval.toLatin1().data());
          num = dq.toQDecNumber();
          return true;
        }
      } // end switch
    ...
    if(altn.exactMatch(token)) {
      ...
      switch(fmtsize) {
        case 32: {
          ...
          QDecSingle ds(val.toLatin1().data());
          num = ds.toQDecNumber();
          return true;
        }
        ...
        case 64: {
          qDebug() << "fmt=" << fmt << "val=" << val;
          QDecDouble dd(val.toLatin1().data());
          num = dd.toQDecNumber();
          return true;
        }
  ...
  QDecNumber tnum;
  tnum.fromString(tt.toLatin1().data(), &c);
  num = tnum;
...
void QDecNumberTests::test_cases()
{
  ...
  try {
    ...
    qDebug() << "testfilefilter=" << tffilter.toLatin1();      
    if(tffilter.size()) {
      QStringList filters;
      filters << tffilter;
    ...
    for(i = 0; i < list.size(); ++i) {
      ...
      if(tfile.size() && tfile!=list[i]) {
        qWarning() << "Skipping " << list[i].toLatin1();
        continue;
      }
      qDebug() << list[i].toLatin1();      
